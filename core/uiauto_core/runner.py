...
from .interfaces import ISession, IActions
...
class Runner:
    ...
    Framework-agnostic runner that accepts ISession and IActions implementations.
    """
    ...
    def run(
        ...
        """
        Run a scenario using provided session and actions implementations.
        
        Args:
            scenario_path: Path to scenario YAML file
            session: ISession implementation (framework-specific)
            actions: IActions implementation (framework-specific)
            app_path: Optional application path to start
        ...
        try:
            # If app_path provided explicitly, start the application
            if app_path:
            ...
            for idx, step in enumerate(steps, start=1):
                ...
                try:
                    self._execute(keyword, args, session, actions)
    ...
    def _execute(self, keyword: str, args: Dict[str, Any], session: ISession, actions: IActions) -> None:
        """
        ...
        if keyword == "open_app":
            path = args["path"]
            session.start(path, wait_for_idle=bool(args.get("wait_for_idle", False)))
            return
        ...
        if keyword == "kill_app":
            # For kill_app, we need to handle it differently since it's session-specific
            # Framework implementations should add a kill method to their session
