...
class Runner:
    ...
    def run(
        ...
        """
        Run a scenario using provided component factories.
        
        Args:
            scenario_path: Path to scenario YAML file
            session_factory: Callable that returns ISession instance
            resolver_factory: Callable(session, repo) that returns IResolver instance
            actions_factory: Callable(resolver) that returns Actions instance
            app_path: Optional path to application to start
            variables: Optional variable overrides
            report_path: Optional path to save JSON report
            
        Returns:
            Report dict with scenario execution results
        """
        ...
        # Build session/resolver/actions using factories
        ...
        try:
            ...
            resolver = resolver_factory(sess, self.repo)
            actions = actions_factory(resolver)
