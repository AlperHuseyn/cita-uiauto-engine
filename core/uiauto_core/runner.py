...
from uiauto_core.exceptions import UIAutoError
...
class Runner:
    ...
    def run(
        ...
        scenario_path: str,
        session: ISession,
        resolver: Any,  # IResolver - kept as Any to avoid circular import
        actions: Any,   # IActions - kept as Any to avoid circular import
        app_path: Optional[str] = None,
        variables: Optional[Dict[str, Any]] = None,
        report_path: Optional[str] = None,
    ) -> Dict[str, Any]:
        """
        Run a scenario using provided session, resolver, and actions.
        
        @param scenario_path Path to scenario YAML file
        @param session Session instance (framework-specific)
        @param resolver Resolver instance (framework-specific)
        @param actions Actions instance (framework-specific)
        @param app_path Optional app path to start
        @param variables Optional variables for scenario
        @param report_path Optional path to save report JSON
        @return Report dict
        """
        ...
        try:
            # If app_path provided explicitly, start it
            if app_path:
